...
The current implementation is the **v2 architecture**, which extends the original LLLite with:
...
> **Compatibility note.** v2 weights use a different `state_dict` from the earlier (pre-v2) experimental implementation; old weights cannot be loaded into the v2 code and will produce an explicit error. Re-train with the current code. The on-disk weight key format is also self-describing (sd-scripts LoRA-style): per-module keys are prefixed by the target Linear's full path (e.g. `lllite_dit_blocks_0_self_attn_q_proj.down.weight`), the shared conditioning encoder is under `lllite_conditioning1.*`, and per-module depth embeddings are stored as `{name}.depth_embed`. Earlier in-development snapshots that used `lllite_modules.{i}.*` keys are rejected on load.
...
現在の実装は **v2 アーキテクチャ**で、初期実装に対して以下を拡張しています：
...
> **互換性に関する注意:** v2 の重みは旧実装と `state_dict` 構造が異なります。古い重みを v2 のコードでロードすると明示的なエラーで失敗します。現行コードで再学習してください。重みファイル上のキー命名も自己記述的な形式（sd-scripts の LoRA と同じスタイル）になっており、各 LLLite モジュールのキーには対象 Linear のフルパス（例: `lllite_dit_blocks_0_self_attn_q_proj.down.weight`）が prefix として付き、共有 conditioning encoder は `lllite_conditioning1.*`、各モジュールの depth embedding は `{name}.depth_embed` に保存されます。開発中のスナップショットで使われていた `lllite_modules.{i}.*` 形式の重みは load 時に reject されます。
...
conditioning1.conv1.{weight,bias},  conditioning1.norm1.{weight,bias}
conditioning1.conv2.{weight,bias},  conditioning1.norm2.{weight,bias}
conditioning1.conv3.{weight,bias},  conditioning1.norm3.{weight,bias}

# (optional) ResBlocks at trunk width
conditioning1.resblocks.{j}.norm1.{weight,bias}, conditioning1.resblocks.{j}.conv1.{weight,bias}
conditioning1.resblocks.{j}.norm2.{weight,bias}, conditioning1.resblocks.{j}.conv2.{weight,bias}

# (optional, if --lllite_use_aspp) ASPP tail
conditioning1.aspp.branches.{k}.0.{weight,bias}, conditioning1.aspp.branches.{k}.1.{weight,bias}
conditioning1.aspp.global_conv.0.{weight,bias},  conditioning1.aspp.global_conv.1.{weight,bias}
conditioning1.aspp.proj.0.{weight,bias},         conditioning1.aspp.proj.1.{weight,bias}

# final 1x1 projection to cond_emb_dim, then LayerNorm on (B, S, cond_emb_dim)
conditioning1.proj.{weight,bias}
conditioning1.out_norm.{weight,bias}
